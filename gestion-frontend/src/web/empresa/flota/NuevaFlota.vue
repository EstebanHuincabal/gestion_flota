...
const guardar = async () => {
  ...
  try {
    const res  = await apiFetchEmpresa('/api/empresa/flotas/', { method: 'POST', body: form.value })
    const data = await res.json()
    if (!res.ok) {
      if (data.error === 'Sin permisos.') return
      if (typeof data === 'object' && !data.error) errores.value = data
      else error.value = data.error || 'Error al crear la flota'
      return
    }
...
<template>
  <div class="page">
    ...
    <h1 class="page-title">Nueva Flota</h1>
    <p class="page-subtitle">Agrupa vehículos bajo un nombre de flota</p>
    <div class="card">
      ...
      <form @submit.prevent="guardar" class="form">
        ...
        <div class="form-actions">
          ...
          <button type="submit" class="btn-primary" :disabled="guardando">
            ...
            {{ guardando ? 'Guardando...' : 'Crear Flota' }}
          </button>
        </div>
      </form>
    </div>
